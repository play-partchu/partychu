...
/**
 * 마이페이지 "로그인 계정" 표시용 — 이번 로그인에서 소셜이 돌려준 이메일을
 * `users/{uid}.socialAccount`에 서버 권한으로 merge한다.
 *
 * Auth email을 쓰지 않고 따로 두는 이유:
 *  · Auth email은 계정을 **처음 만들 때만** 들어가고(위 issueCustomToken),
 *    다른 계정이 쓰는 이메일이면 아예 비워 둔다. 기존 가입자는 소셜 쪽에서
 *    이메일 동의를 새로 해도 영영 채워지지 않는다.
 *  · 그 동작은 계정 병합·중복 정책과 얽혀 있어 건드리지 않는다. 표시용 값은
 *    여기에 **매 로그인마다** 덮어써서 기존 가입자도 다음 로그인부터 채워진다.
 *
 * 이메일이 없으면 null로 덮는다 — 사용자가 소셜 쪽에서 이메일 제공을 철회했는데
 * 예전 값을 계속 들고 있으면 안 된다.
 *
 * **실패해도 던지지 않는다.** 표시용 부가 정보 때문에 로그인이 막히면 안 된다.
 * 로그에는 이메일·uid·토큰을 남기지 않는다.
 * 탈퇴 시 삭제는 accountWithdrawal.js의 PERSONAL_FIELDS가 맡는다.
 */
async function recordSocialAccount(uid, provider, email, deps = {}) {
  try {
    // deps는 셀프체크가 가짜 DB를 넣는 자리다. 운영에서는 비워 둔다.
    ...
    console.warn(
      `[socialAuth] socialAccount 저장 실패(로그인은 계속) provider=${provider} ` +
      `hasEmail=${Boolean(email)} code=${e.code || ''}`,
    );
  }
}
...
exports.kakaoCustomToken = onCall(
  ...
  async (request) => {
    ...
    // 토큰이 발급된 뒤에만 기록한다. 실패해도 던지지 않는다.
    await recordSocialAccount(`kakao:${me.body.id}`, 'kakao', email);
...
exports.naverCustomToken = onCall(
  ...
  async (request) => {
    ...
    // 네이버는 로그인 아이디를 API로 주지 않는다 — 이메일이 계정 식별값이다.
    // 토큰이 발급된 뒤에만 기록한다. 실패해도 던지지 않는다.
    await recordSocialAccount(`naver:${r.id}`, 'naver', r.email || '');
