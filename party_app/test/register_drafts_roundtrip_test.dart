...
void main() {
  ...
  testWidgets('이벤트: 입력 → 임시저장 버튼 → draft_event 로컬 미러 기록', (tester) async {
    await tester.pumpWidget(const MaterialApp(home: EventRegisterScreen()));
    await tester.pumpAndSettle();

    await tester.enterText(fieldWithHint(kEventNameHint), '주말 플리마켓');
    await tester.pump();
  ...
  testWidgets('이벤트: 임시저장 있으면 재진입 시 이어서 작성으로 복구', (tester) async {
    SharedPreferences.setMockInitialValues({
    ...
    expect(textInField(tester, kEventNameHint), '복구 이벤트');

  });

  testWidgets('파티크루: 기본(구인) 입력 → draft_crew_recruit에 기록', (tester) async {
    await tester.pumpWidget(const MaterialApp(home: CrewRegisterScreen()));
    await tester.pumpAndSettle();

    // 제목 칸은 힌트로 찾는다 — 폼 위쪽에 다른 입력칸이 생겨도 따라간다.
    await tester.enterText(fieldWithHint(kCrewRecruitTitleHint), '스탭 급구');
    await tester.pump();
    await tester.tap(find.widgetWithText(TextButton, '임시저장'));
    await tester.pumpAndSettle();
    ...
    final payload =
        (jsonDecode(prefs.getString('draft_crew_recruit')!)
  ...
  testWidgets('파티크루: 구직 임시저장을 initialCrewType으로 복구', (tester) async {
    SharedPreferences.setMockInitialValues({
    ...
    expect(textInField(tester, kCrewSeekTitleHint), '구직합니다');
  });
}
