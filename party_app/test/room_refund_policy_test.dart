...
void main() {
  testWidgets('RefundPolicyView는 subjectLabel로 맥락에 맞게 표시된다', (tester) async {
    final tiers = [
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: RefundPolicyView(tiers: tiers, subjectLabel: '이용'),
        ),
      ),
    );
    await tester.pump();
  ...
  testWidgets('RefundPolicyView 기본 subjectLabel은 파티 그대로(회귀 방지)', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: RefundPolicyView(
            tiers: [RefundTier(daysBefore: 5, refundPercent: 50)],
          ),
        ),
      ),
    );
  ...
  testWidgets(
    'RoomCard: refundPolicy 있는 룸은 "N단계 환불 규정 설정됨" 표시 + getDraftData 왕복',
    (tester) async {
  ...
  testWidgets('RoomCard: 구 스키마(cancelPolicy 문자열)만 있으면 안내로 보존', (tester) async {
    final key = GlobalKey<RoomCardState>();
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
