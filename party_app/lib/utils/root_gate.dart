...
RootGateScreen resolveRootGateScreen({
  ...
  bool allowUnverifiedMember = false,
}) {
  ...
  switch (identityStatus) {
    ...
    case IdentityStatus.unverified:
      // 소셜 로그인 테스트 모드(Android Debug 전용)에서만 통과시킨다 —
      // [socialLoginTestMode] 참고. 기본값이 false라 운영 판정은 그대로다.
      return allowUnverifiedMember
          ? RootGateScreen.main
...
RootGateScreen currentRootGateScreen() => resolveRootGateScreen(
  ...
  allowUnverifiedMember: socialLoginTestMode,
);

/// **소셜 로그인 테스트 모드** — Android Debug(flutter run) 빌드에서만 true.
///
/// 왜 필요한가: 파티츄는 "1명 = 계정 1개"라 본인확인(NICE)의 CI를 계정 하나에만
/// 묶는다(functions/identityLink.js). 그런데 소셜 로그인은 구글·카카오·네이버마다
/// 서로 다른 Firebase 계정(uid)을 만든다. 한 개발자가 두 번째 제공자로 로그인하면
/// 그 계정은 미인증 → 루트 게이트가 본인확인 화면에 가두고 → 본인확인은 CI 중복으로
/// 서버가 거부한다. OAuth는 성공했는데 앱에 들어갈 길이 없는 막다른 길이 된다.
///
/// 그래서 **클라이언트의 화면 게이트만** 연다. 우회하지 않는 것:
///  · 서버의 CI 중복 차단(identityLink.js) — 서버는 앱의 디버그 여부를 믿을 수
///    없고, 그걸 뚫으면 운영 계정의 본인확인 연결이 오염된다.
///  · 회원 기능의 서버 검사(identityGuard.js, firestore.rules의
///    isVerifiedMember()) — 미인증 테스트 계정의 신청·등록 같은 쓰기는 그대로
///    거부된다. 둘러보기와 로그아웃까지만 된다.
///
/// kDebugMode는 컴파일 상수라 Release/Profile에서는 이 식이 false로 접혀 코드가
/// 남지 않는다. iOS·웹은 이번 범위 밖이라 제외했다.
bool get socialLoginTestMode =>
    kDebugMode && !kIsWeb && defaultTargetPlatform == TargetPlatform.android;

/// 회원 기능 진입에 필요한 본인확인 조건을 만족하는가.
///
/// 루트 게이트와 **같은 기준**을 하단 탭 진입(main_screen.dart)에서도 쓰기 위한
/// 것이다. 두 곳이 따로 판단하면 게이트는 열렸는데 마이 탭(로그아웃)에는 못
/// 들어가는 식으로 어긋난다.
bool identityRequirementMet(IdentityStatus status) =>
    status == IdentityStatus.verified ||
    (socialLoginTestMode && status == IdentityStatus.unverified);
