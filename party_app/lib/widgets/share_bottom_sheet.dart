...
class _ShareBottomSheet extends StatelessWidget {
  ...
  Future<void> _shareKakao(String url) async {
    try {
      ...
      // (또는 없는) 사진이 뜨던 문제가 없다.
      final coverImage = getPartyCoverMedia(
        partyData,
        tag: 'KakaoShare',
      )?.thumbnailUrl;
      ...
      final template = FeedTemplate(
        content: Content(
          ...
          imageUrl: coverImage != null && coverImage.isNotEmpty
  ...
  Future<void> _shareMore(String url, {bool silent = false}) async {
    try {
      await SharePlus.instance.share(
        ShareParams(text: ShareService.buildShareText(partyData, url)),
      );
      ShareService.logShareEvent(partyId);
    } catch (e) {
        ...
        child: Column(
          ...
          children: [
            ...
            GridView.count(
              ...
              children: [
                for (final option in options) _ShareOptionTile(option),
              ],
            ),
          ],
        ),
