...
class _MyPageScreenState extends State<MyPageScreen> {
  ...
  Widget _buildProfileHeader() {
    ...
    // 로그인 계정 — 프로필 바로 아래, 같은 흰 카드 안에 둔다(회원정보 영역).
    final account = loggedIn ? currentLoginAccount() : null;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          _buildProfileRow(loggedIn, verified, hasNickname, hasPhoto),
          if (account != null) ...[
            const SizedBox(height: 16),
            const Divider(height: 1),
            const SizedBox(height: 14),
            _LoginAccountRow(account: account),
          ],
        ],
      ),
    );
                    ...
                    Flexible(
                      child: Text(
                        ...
                        style: TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.bold,
                          color: hasNickname ? Colors.black87 : Colors.black38,
                        ),
                        maxLines: 1,
...
/// 마이페이지 프로필 아래 **로그인 계정** 한 줄 — 어느 소셜 계정으로
/// 가입·로그인했는지 사용자가 직접 확인하는 자리다(Release 포함).
///
/// 보여주는 것은 제공자 이름과 이메일뿐이다. UID·소셜 내부 id·토큰은
/// [LoginAccount]에 애초에 담기지 않는다. 이메일이 없으면(카카오·네이버의
/// 이메일 동의는 선택) "카카오 계정으로 로그인됨"으로 대신한다.
class _LoginAccountRow extends StatelessWidget {
  const _LoginAccountRow({required this.account});
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        _ProviderMark(provider: account.provider),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                account.hasEmail ? account.email : account.signedInText,
...
/// 제공자 표시 — 로그인 화면 버튼의 로고 모양·색과 맞춘다(login.dart).
class _ProviderMark extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final (Color bg, Color? border, Widget mark) = switch (provider) {
      ...
      LoginProvider.kakao => (
        ...
        null,
        const Icon(Icons.chat_bubble_rounded, size: 16, color: Colors.black),
      ),
      LoginProvider.naver => (
      ...
      LoginProvider.email => (
        ...
        null,
        const Icon(Icons.mail_outline, size: 17, color: Colors.black54),
      ),
    };
    return Container(
      width: 34,
