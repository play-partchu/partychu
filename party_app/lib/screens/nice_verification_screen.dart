...
class _NiceVerificationScreenState extends State<NiceVerificationScreen> {
  ...
  Future<void> _requestAuthUrl() async {
    ...
    try {
      ...
      debugPrint('[NICE] ❌ FirebaseFunctionsException'
          ' code=${e.code}'
          ' message=${e.message}'
          ' details=${e.details}');
      ...
      final String errMsg;
      if (e.code == 'not-found') {
        errMsg = 'Firebase Functions가 배포되어 있지 않습니다.\n\n'
    ...
    ctrl = WebViewController()
      ...
      ..setNavigationDelegate(NavigationDelegate(
        ...
        onHttpError: (HttpResponseError error) {
          debugPrint('[NICE] HTTP 오류(onHttpError)'
              ' statusCode=${error.response?.statusCode}'
              ' url=${error.request?.uri}');
        },
        onNavigationRequest: (NavigationRequest req) {
          debugPrint('[NICE] 내비게이션 요청(onNavigationRequest): ${req.url}'
          ...
          if (req.url.startsWith(_kNiceReturnUrlPrefix)) {
            ...
            debugPrint('[NICE] web_transaction_id 존재=${webTransactionId != null}');
        ...
        onWebResourceError: (WebResourceError err) {
          debugPrint('[NICE] WebResourceError'
  ...
  Future<void> _completeVerification(String webTransactionId) async {
    ...
    try {
      ...
      debugPrint('[NICE] ❌ niceIntcResult 오류: code=${e.code} message=${e.message}');
      if (!mounted) return;
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('본인확인 처리 중 오류가 발생했습니다. (${e.runtimeType})'),
      ));
      setState(() => _completing = false);
    }
  }
